...
func mcpPageHandler(w http.ResponseWriter, r *http.Request) {
	...
	if wallet.X402Enabled() {
		...
		b.WriteString(`Create a <a href="/token">Personal Access Token</a>, or call the <code>login</code> tool to get one programmatically.</li>`)
		b.WriteString(`</ol>`)
		...
		b.WriteString(`<li><strong>Login</strong> &mdash; the agent can call the <code>login</code> tool to obtain a session token programmatically.</li>`)
		b.WriteString(`</ol>`)
	}
	b.WriteString(`</div>`)
