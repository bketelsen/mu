...
func init() {
	...
	Endpoints = append(Endpoints, &Endpoint{
		...
		Path:        "/agent",
		Method:      "POST",
		Description: "Query the AI agent. The agent plans and executes tool calls (GitHub, news, weather, places, and more) then synthesizes a response. Requires authentication. Returns a Server-Sent Events stream.",
		Params: []*Param{
	...
	Endpoints = append(Endpoints, &Endpoint{
		...
		Path:        "/mcp",
		Method:      "POST",
		Description: "Owner-authenticated Model Context Protocol server for AI tool integration. Tools include GitHub, chat, news, blog, video, mail, search, weather, and places.",
		Params: []*Param{
			{
				Name:        "jsonrpc",
				Value:       "string",
